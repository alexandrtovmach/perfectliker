(function($) {
	...
	// Timer likes
		...
		function countdownTimer(startTime, element, name) {
			...
			window["countdownTimer-" + name] = setInterval(() => {
				if (testToFinish(time)) {
					time = tickTack(time);
					element.innerHTML = `${time[0].toString().length === 1? "0" + time[0]: time[0]}:${time[1].toString().length === 1? "0" + time[1]: time[1]}`
				} else {
					clearInterval(window["countdownTimer-" + name]);
					window["countdownTimer-" + name] = null;
				}
			}, 1000);

			function testToFinish(timeArr) {
				return Number(timeArr[0] + timeArr[1]) !== 0;
			}

			function tickTack(timeArr) {
